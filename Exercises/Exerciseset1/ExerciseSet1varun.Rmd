...
<br>
"PerformanceAnalytics": This package contains a large number of functions for portfolio performance calculations and risk management
<br>
...
<br>
This is done by sourcing the file dataSet1.R as well as some helper functions in herpersSet1.R
<br>
<br>
Here are some basic statistics about the AAPL returns:
...
3. The standard deviation of the daily returns of the AAPL index during this period is `r round(100*sd(StockReturns[,2]),3)`%;
<br>

Here are some basic statistics about the YHOO returns:

1. The cumulative returns of the YHOO index during this period is `r round(100*sum(StockReturns[,3]),1)`%.
2. The average daily returns of the YHOO index during this period is `r round(100*mean(StockReturns[,3]),3)`%;
3. The standard deviation of the daily returns of the YHOO index during this period is `r round(100*sd(StockReturns[,3]),3)`%;
<br>
...
<br>
<br>
   Number of Rows: nrow(transposedData): `r nrow(transposedData)` 
   <br>
   Number of Columns: ncol(transposedData): `r ncol(transposedData)`
<br>
<br>
Answer 2
<br>
...
```{r, eval=TRUE, echo = TRUE, runtime: shiny}
